...
//        unset($res[$limit]);
        ...
        //设置默认选中的
        $pos_task_type = intval($_REQUEST['pos_task_type']);
        $type = empty($pos_task_type) ? 1 : intval($_REQUEST['pos_task_type']);
...
//        $arr['ord_headh']=$_SESSION['he_id'];
        ...
        if ($res && $sbs) {
            ...
            $this->display('tast_detail_receive');
        } else {
            $this->assign('data', $data);
            $this->display('tast_detail');
        }
    ...
    function extend()
    {
        ...
        $this->assign('data', $data);
        $link = "/qrcode.php?url=http://" . $_SERVER['HTTP_HOST'] . U('User/Index/index') . "?id={$data['he_id']}";
        $this->assign('codelink', $link);
        $this->display('two_code');
