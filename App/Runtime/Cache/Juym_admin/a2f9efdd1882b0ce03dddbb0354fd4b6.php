    ...
    <div class="am-offcanvas-bar admin-offcanvas-bar">
      ...
      <li class="admin-parent">
        ...
        <ul class="am-list am-collapse admin-sidebar-sub " id="collapse-nav2">
          ...
          <li><a href="<?php echo U('Poscha/lists_j');?>" class="am-cf"><span class="am-icon-slideshare"></span>实习列表<span class="am-icon-star am-fr am-margin-right "></span></a></li>
          <li><a href="<?php echo U('Poscha/lists_s');?>" class="am-cf"><span class="am-icon-slideshare"></span>全职列表<span class="am-icon-star am-fr am-margin-right "></span></a></li>
        </ul>
      </li>
        ...
        <div class="am-u-sm-12 am-u-md-6">
          <div class="am-btn-toolbar">
            <div class="am-btn-group am-btn-group-xs">
              <a href="<?php echo U('Poscha/adds_j',array('id'=>$id));?>"><button type="button"; class="am-btn am-btn-default"><span class="am-icon-plus"></span>添加</button></a>
            </div>
          </div>
        </div>
      </div>
          ...
          <form class="am-form">
            <table class="am-table am-table-striped am-table-hover table-main">
              ...
              <?php if(is_array($data)): foreach($data as $key=>$v): ?><tr>
                ...
                <td>
                      <?php
 $db=M('company'); $map['com_id']=$v['pos_company_id']; $datacom=$db->where($map)->select(); foreach($datacom as $datacoms){ echo $datacoms['com_name']; } ?>
                </td>
                <td class="img_show">
                    <?php if($v['pos_img'] != '' ): ?><img style="height:70px;width:90px" src="<?php echo ($v['pos_img']); ?>">
                     <?php else: ?>无<?php endif; ?>
                </td>
                <td><?php echo ($v['pos_recruitmun']); ?></td>
                <td>
                    <?php
 $db=M('pos'); $map['pos_id']=$v['pos_county']; $data=$db->where($map)->select(); foreach($data as $datas){ echo $datas['pos_name']; } ?>
                </td>
                <td><?php echo ($v['pos_brokerage']); ?></td>
                <td><?php echo ($v['pos_salary']); ?></td>         
                <td>
                    <?php
 $db=M('pos'); $map['pos_id']=$v['pos_joptype']; $data=$db->where($map)->select(); foreach($data as $datas){ echo $datas['pos_name']; } ?>
                </td>
                <td><?php echo ($v['pos_worktime']); ?></td>
                <td>
                    <a href="javascript:;" data-value="<?php echo ($v['pos_online']); ?>" class="btnRec">
                    <?php  $db = M('orderinfo'); $map['ord_poschaid']=$v['pos_id']; $data=$db->where($map)->count(); if($data==$v['pos_recruitmun']){ $v['pos_online']=0; } echo $v['pos_online']?'上架':'下架';?></a>
                </td>
                <td>
                    <开启/关闭>
                </td>
                <!-- <td><?php echo ($v['pos_desc']); ?></td> -->
                <td>
                    <?php
 $db = M('orderinfo'); $map['ord_poschaid']=$v['pos_id']; $data=$db->where($map)->count(); echo $data; ?>
                </td>
                <td>
                  <div class="am-btn-toolbar">
                    <div class="am-btn-group am-btn-group-xs">
                      <button type="submit" formmethod="get" formaction="<?php echo U('Poscha/view',array(id=>$v['pos_id']));?>" class="am-btn am-btn-default am-btn-xs am-text-secondary"><span class="am-icon-pencil-square-o"></span>查看</button>
...
<script type="text/javascript">
 ...
 $(document).on('click','.btnRec',function(){
        ...
        var pos_online = $(this).attr('data-value');
        if(pos_online==1)
            value = 0
        else
            value = 1
        $.ajax({
            url:"<?php echo U('poscha/doonline');?>",
            data:{
                "id":id,
                "pos_online":value,
            },
