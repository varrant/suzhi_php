          ...
          <form class="am-form">
            <table class="am-table am-table-striped am-table-hover table-main">
              ...
              <?php if(is_array($data)): foreach($data as $key=>$v): ?><tr>
                <td><?php echo ($v['he_id']); ?></td>
                <td><?php echo ($v['he_contact']); ?></td>
                <td><img src="<?php echo ($v['he_image']); ?>"style="width:40px;height:40px" ></td>
                <td><?php echo ($v['he_nickname']); ?></td>
                <?php if($v['he_sex'] == '0'){ echo "<td>男</td>"; }elseif($v['he_sex'] == '1'){ echo "<td>女</td>"; }else{ echo "<td>暂无</td>"; } ?>
                <td><?php echo ($v['he_name']); ?></td>
                <td><?php echo ($v['he_occupation']); ?></td>
                <td><?php echo ($v['l_phone']); ?></td>
                <td><?php echo ($v['l_nick']); ?></td>
                <td><?php echo ($v['l_name']); ?></td>
                <td>
                  <div class="am-btn-toolbar">
                    <div class="am-btn-group am-btn-group-xs">
                      <button type="submit" formmethod="get" formaction="<?php echo U('Head/qz_view',array(id=>$v['he_id']));?>" class="am-btn am-btn-default am-btn-xs am-text-secondary"><span class="am-icon-pencil-square-o"></span>查看个人信息</button>
                      <button type="submit" formmethod="get" formaction="<?php echo U('Order/enlist_list',array(id=>$v['he_id']));?>" class="am-btn am-btn-default am-btn-xs am-text-secondary"><span class="am-icon-pencil-square-o"></span>查看订单</button>
...
<script type="text/javascript">
  function ssss(dom) {
   //获取要删除的ID
    ...
    $.ajax({
      url:"<?php echo U('head/qz_delete');?>",
      data:data,
      type:'post',
      dataType:"json",
      success:function(data){
        if(data.status){
          window.location="<?php echo U('head/qz_lists');?>";
        }else{
          alert(data.message);
        }
      }
