...
<body style="background:#eee;">
		...
		<div class="mission_top">
			<div class="top_head">
				<div style="overflow:hidden;">
					...
					<span style=" color: #ff8903;"><?php echo ($data['pos_salary']); ?></span>
				</div>
 ...
 $where['pos_id']=$data['pos_joptype']; $db=M('pos'); $res=$db->where($where)->find(); echo $res['pos_name']; ?>
		...
		<div class="tast_footer">
			...
			<a href="javascript:receive_task()" class="receive_task">立即报名</a>
		</div>
	</div>
	<!--弹框-->
    ...
    <!--弹框-->
    <div class="receive_task_bg" id="send_resume" style="display:none;">
        <div class="receive_task_box">
            <h3>请投递简历至jianli@suzhi.com</h3>
            <div class="receive_task_link">
                <a onclick="send_resume_hide();" style="width: 97%;">确认</a>
            </div>
        </div>
    </div>
...
<script>
    ...
    function send_resume_show(){
        $("#send_resume").show();
    }
    function send_resume_hide(){
        $("#send_resume").hide();
        //
        reload_url(jump_url);
    }
	...
	function queren() {
		...
		var ord_jobseekerid=$('#ord_jobseekerid').val();
		var data={'ord_poschaid':ord_poschaid,'ord_jobseekerid':ord_jobseekerid};
		$.ajax({
