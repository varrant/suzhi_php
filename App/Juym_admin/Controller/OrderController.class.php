...
class OrderController extends CommonController {
  ...
  function enlist_list(){
    ...
    $show = $Page->show();// 分页显示输出
    $show=substr($show,0,strlen($show)-1);
    $data = $db->field("sz_orderinfo.ord_id,sz_orderinfo.order_id,sz_orderinfo.ord_jobseekerid,sz_poscha.pos_name,sz_poscha.pos_worktime,sz_poscha.pos_salary,sz_poscha.pos_brokerage,sz_headhunter.he_contact,sz_headhunter.he_nickname,sz_headhunter.he_name,sz_headhunter.he_hunterid" )->join( "sz_poscha on sz_orderinfo.ord_poschaid=sz_poscha.pos_id" )->join( "sz_headhunter on sz_orderinfo.ord_jobseekerid=sz_headhunter.he_id" )->where($map)->limit($Page->firstRow.','.$Page->listRows)->select(); // 进行分页数据查询 注意limit方法的参数要使用Page类的属性
    foreach ($data as $key => $value){
     $where['he_id']=$value['he_hunterid'];
     $res=$head->where($where)->find();
     $data[$key]['q_phone']=$res['he_phone'];
  ...
  function enlist_details(){
    ...
    $head=M('headhunter');
    $map['ord_id']=I('id');
    $data = $db->field("sz_orderinfo.ord_id,sz_orderinfo.order_id,sz_orderinfo.ord_jobseekerid,sz_poscha.*,sz_headhunter.he_contact,sz_headhunter.he_nickname,sz_headhunter.he_name,sz_headhunter.he_hunterid" )->join( "sz_poscha on sz_orderinfo.ord_poschaid=sz_poscha.pos_id" )->join( "sz_headhunter on sz_orderinfo.ord_jobseekerid=sz_headhunter.he_id" )->where($map)->select();
    foreach ($data as $key => $value){
      $where['he_id']=$value['he_hunterid'];
      $res=$head->where($where)->find();
      $data[$key]['q_phone']=$res['he_phone'];
