...
class Common_util_pub
{
	...
	{		
		...
		//var_dump(curl_setopt($ch, CURLOPT_TIMEOUT, $second));
        //这里设置代理，如果有的话
		...
		//var_dump(curl_setopt($ch, CURLOPT_POSTFIELDS, $xml));
		//运行curl  htmlspecialchars
        $data = curl_exec($ch);
...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	function postXml()
	{
		...
		// var_dump($this->postXmlCurl($xml,$this->url,$this->curl_timeout));
	...
	function createXml()
	{
		try
		{
			...
			if($this->parameters["out_trade_no"] == null && $this->parameters["transaction_id"] == null) {
		   	...
		   	$this->parameters["mch_id"] = WxPayConf_pub::MCHID;//商户号
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
		    var_dump($this->getSign($this->parameters));
		    $this->parameters["sign"] = $this->getSign($this->parameters);//签名
		    return  $this->arrayToXml($this->parameters);
		}catch (SDKRuntimeException $e)
		{
